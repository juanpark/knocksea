...
def merge_excels_to_csv(folder_path, output_csv):
    ...
    for file in excel_files:
        ...
        try:
            ...
            road_col = df['소재지도로명주소'] if '소재지도로명주소' in df.columns else pd.Series([pd.NA]*len(df))
            lot_col = df['소재지지번주소'] if '소재지지번주소' in df.columns else pd.Series([pd.NA]*len(df))
            ...
            # 시/도 및 시군구(전체) 추출
            df[['시/도', '시군구(전체)']] = df['주소'].str.extract(r'^(\S+)\s+(\S+)')
    ...
    print(f"모든 엑셀 파일을 병합하여 '{OUTPUT_EXCEL}'로 저장했습니다.")

if __name__ == "__main__":
    merge_excels_to_csv(FOLDER_PATH, OUTPUT_EXCEL)